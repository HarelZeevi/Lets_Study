...
const tests = require("../../../test/testInput");
...
function signJwt(result, err, res) {
  if (err) {
    ...
    if (Object.keys(result).length != 0) {
      ...
      if (uType === "T") result[0].isTeacher = true;
      else result[0].isTeacher = false;
...
function sendToken(res, result) {
  ...
  smtpTransport.sendMail(mailOptions, function (error, info) {
    if (error) {
      ...
      sqlQuery = `UPDATE students SET token = ${mysql.escape(
        token
      )}, expiration = ${mysql.escape(expiration)} WHERE id = ${mysql.escape(
        studentId
      )};`;
...
function testData(value, inputType) {
  ...
  switch (inputType) {
    case 1: { // name
      if (!tests.validateName(value)) return 1;
    }
    case 2: { // pswd
      if (!tests.validatePswd(value)) return 2;
      break;
    }
    case 3: { // username
      if (!tests.validateUsername(value)) return 3;
      break;
    }
    case 4: { //phone
      if (!tests.validatePhone(value)) return 4;
      break;
    }
    case 5: { // id
      if (!tests.validateId(value)) return 5;
      break;
    }
    case 6: { // email
      if (!tests.validateEmail(value)) return 6;
      break;
    }
    case 7: { //
      // check here
...
function base64Img(id, ext) {
  let file =
    "C:/ProgramData/MySQL/MySQL Server 8.0/Uploads/profileImages/" + id + ext;
  // read binary data
  try {
  ...
  get_profile_img(req.tokenData.id, function (extension) {
    ...
    if (base64Data != null) {
      if (base64Data.indexOf("dataimage/") == -1)
        base64Data =
          `data:image/${extension.replace(".", "")};base64,` +
          base64Img(req.tokenData.id, extension);
      else
...
app.get("/api/isTeacher", authJwt, (req, res) => {
  let isTeacher = false;
  if (req.tokenData.userType == "T") isTeacher = true;
  res.send({
    isTeacher: isTeacher,
  });
});
...
app.post("/api/getAvailability/", authJwt, (req, res) => {
  let tutorId;
  if (req.tokenData.userType == "P") tutorId = req.body.tutorId;
  else tutorId = req.tokenData.id;
  console.log(tutorId);
...
app.post("/api/sendToken/", (req, res) => {
  ...
  findStudent(email, studentId, (result) => {
...
app.post("/api/checkToken/", (req, res) => {
  ...
  checkToken(token, (result) => {
    if (
      ...
      compareTimes(result[0].expiration)
    ) {
