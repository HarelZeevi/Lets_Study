...
function searchTeacher(res, subjectNum, date, studentGender, tutorGender, grade1, grade2 = undefined, rate, offset) {
    ...
    var limit = `GROUP BY(tutors.studentid) LIMIT ${mysql.escape(parseInt(offset))}, 5 `;
    if (grade1 === undefined) // No grade preferences
        sqlQuery += limit + ";";
    else
        sqlQuery += gradesPrefs + limit + ";";
...
app.post('/api/findTutors/', authJwt, (req, res) => {
    ...
    searchTeacher(res, subjectNum, date, studentGender, tutorGender, grade1, grade2, rate, offset);
});
