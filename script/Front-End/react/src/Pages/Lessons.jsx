...
function Lessons() {
  ...
  const [filterAll, setFilterAll] = useState(true); // this variable determines wheather filter all was the function that was actiavated last click

  // input variables for request to server
  const date = null;
  let tutorGender = null;

  const offset = useRef(0); // the offset inside the table of the available tutors
  ...
  useEffect(() => {
    ...
    if (mount) {
      ...
      fetchTeachers({ offset: offset.current }, (res) => {
        //setOffset(offset + 1); // setting s new of
  ...
  const updateGender = (event) => {
    // resetting the offset of the requests
    offset.current = 0;
  ...
  const updateGrade = (event) => {
    // storig the last values of 3 check boxes in order to check for difference
    // if a difference is found, the request ofset will reset

    // since the request's filtering parameters were changed - the ofset counter must be zeroed
    offset.current = 0;
  ...
  const confirm_filter_submit = (event) => {
    ...
    // setting a new offset according to the button which was clicked
    ...
    // normal filter request without 'Next' or 'Prev'. Therefore, the offset is 0.
      ...
      offset.current = 0;
      setFilterAll(false); // a specific filter parameters were given
    }
    ...
    // if there's a specific requst with filters than get the next filtered teachers
  ...
  return (
    <div dir="rtl">
      ...
      <div className="next-prev-btns">
        <button
          className="prev-btn"
          onClick={confirm_filter_submit}
          id="Prev"
          disabled={offset == 0 ? true : false}
        >
          Previous
        </button>
        <button className="next-btn" onClick={confirm_filter_submit} id="Next">
          Next
        </button>
      </div>
      ...
      <div className="sort-filter-container">
        <div
          className="filter2-containter nonselective"
        ...
        <div id="sort-form-container">
          <form className="sort-form" onSubmit={confirm_filter_submit}>
            ...
            >
              <option value="0" className="tf-option-subject-fields">
                בחר מקצוע
              </option>
              <option value="1" className="tf-option-subject-fields">
                אזרחות
              </option>
              <option value="2" className="tf-option-subject-fields">
                ביולוגיה
              </option>
              <option value="3" className="tf-option-subject-fields">
                דינים
              </option>
              <option value="4" className="tf-option-subject-fields">
                היסטוריה
              </option>
              <option value="5" className="tf-option-subject-fields">
                אנגלית
              </option>
              <option value="6" className="tf-option-subject-fields">
                הנדסת תוכנה - אנדרואיד
              </option>
              <option value="7" className="tf-option-subject-fields">
                הנדסת תוכנה - סייבר
              </option>
              <option value="8" className="tf-option-subject-fields">
                הנדסת תוכנה - שירותי רשת
              </option>
              <option value="9" className="tf-option-subject-fields">
                כימיה
              </option>
              <option value="10" className="tf-option-subject-fields">
                לשון
              </option>
              <option value="11" className="tf-option-subject-fields">
                מדעי המחשב
              </option>
              <option value="12" className="tf-option-subject-fields">
                מחשבת ישראל
              </option>
              <option value="13" className="tf-option-subject-fields">
                מתמטיקה
              </option>
              <option value="14" className="tf-option-subject-fields">
                ספרות
              </option>
              <option value="15" className="tf-option-subject-fields">
                ערבית
              </option>
              <option value="16" className="tf-option-subject-fields">
                פיזיקה
              </option>
              <option value="17" className="tf-option-subject-fields">
                תלמוד
              </option>
              <option value="18" className="tf-option-subject-fields">
                תנ"ך
              </option>
            </select>
            ...
            <div className="date-time-continer-sort">
              <input
                type="date"
                onChange={(date) => {
                  setSelectedDate(date.target.valueAsDate);
                }}
              />
            </div>
            <br />
            <hr />
            <button type="submit" className="TF_FilterSubmitBtn">
              סינון
            </button>
          </form>
        </div>
