...
function checkResult(result, err, res)
{
    ...
    else
    {
        ...
        if (Object.keys(result).length != 0)
        {
            ...
            res.send("We didnt find what you are looking for...").status(422);
...
function generateStudentCode(length) {
    ...
    for (let i = 0; i < length; i++) {
        if (Math.random() >= 0.6) {
            ...
            code = code + String.fromCharCode(Math.round(Math.random() * (90 - 33) + 33));
        }
    }
    return code;
}
...
function addStudent(res, id, school, grade, classnum)
{
    const studentCode =  generateStudentCode(10);
    var sqlQuery1 = `
...
function createAdmin(res, id, firstname, lastname, pswd, school)
{
    var sqlQuery = 
    `INSERT INTO admins
    (
...
function cancelLesson(res, lessonId)
{ 
    var sqlQuery1 = `DELETE FROM rates WHERE lessonid = ${mysql.escape(lessonId)}`; // delete lesson rate 
    var sqlQuery2 = `DELETE FROM lessons WHERE id = ${mysql.escape(lessonId)};`;  // delete the lesson itself
    con.query(sqlQuery1,  function(err, result){
    });
    con.query(sqlQuery2,  function(err, result){
        checkResult(result, err, res);
...
app.post('/api/students', (req, res) => {
    ...
    const school = req.body.school; // automatically entered by admin's school
    const grade = req.body.grade; // admin enters info
    const classnum = req.body.classnum; // admin enters info

    addStudent(res, id, school, grade, classnum);
});
...
app.get('/api/students/signIn', (req, res) => {
    const id = req.body.id;
    const username = req.body.username;
...
app.get('/api/admins/:id/:password', (req, res) => {
    const id = req.params.id;
...
app.post('/api/admins', (req, res) => {
    ...
    createAdmin(res, id, firstname, lastname, pswd, school);
});

// look for a teacher
app.get('/api/tutors/:subject/:date/:studentGender/:rate/:tutorGender?/:grade1?/:grade2?', (req, res) => {
...
app.get('/api/availability/:tutorId', (req, res) => {
...
app.delete('/api/lessons/:lessonId', (req, res) => {
    ...
    cancelLesson(res, lessonId);
});

// aprove tutor function
app.put('/api/approveTutor/:tutorId', (req, res) => {
    const tutorId = req.params.tutorId;
...
app.get('/api/lessons/:studentId', (req, res) => {
...
app.delete('/api/teachingSubjects/:tutorId/:subject/:grade/:points', (req, res) => {
...
app.get('/api/tutoringHours/:tutorId', (req, res) => {
    const tutorId = req.params.tutorId;
    getTutoringHours(res, tutorId);
});

// add tutoring hour to tutor
app.post('/api/tutoringHours', (req, res) => {
    const tutorId = req.body.tutorId;
...
app.post('/api/students/uploadProfileImg/', (req, res) => {
